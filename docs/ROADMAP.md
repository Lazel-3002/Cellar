...
### M6 follow-up: drawing on the board step by step (2026-09-22)
From the creator's notebook pages (`docs/Creator Ideas - important things/`): thick, bold, coloured, dotted, dashed and see-through lines, α angles, and tan/cot read off the unit circle, drawn one step at a time the way a teacher does it.
- **Step-by-step diagrams** (`shared/math/diagram.ts`, a new `diagram` block): the model describes a drawing in maths coordinates (axes and number lines, the unit circle, points, segments, infinite lines like `x = 1`, rays, arrows, circles, arcs, angle marks, polygons and shaded regions, function curves, text). Coordinates can be expressions (`cos 135`), worked out by the calculator. Every element belongs to a step with one sentence (and optional maths). The board draws each step in (lines traced, labels faded), with play/pause, previous/next, start again and show all. New steps from the model play automatically, and so does a diagram the model extends with `update_block` + `addSteps`.
- **Line styles** (`shared/math/linestyle.ts`): named colours or #hex (sanitised), thin/normal/bold/thick, solid/dashed/dotted/dash-dot, **zigzag** (the scribbled highlight from the notebook) and wavy, and opacity (alpha). Shared by diagrams, the whiteboard and exports.
- **`draw_diagram` tool** with a **`trig-circle` preset**: Cellar builds the whole construction for sin, cos, tan and cot of an angle (the tangent axis x = 1, the cotangent axis y = 1, OP extended back through O in quadrants II and III, the highlighted segment with its exact value and sign). A list of angles makes one diagram each. The model's own step wording (in the user's language) replaces the default text. Loose input is normalised (`diagram-normalize.ts`): `steps: [{ say, draw }]` or a flat list, aliases like "dashed", "sketched", "vector", "unit circle".
- **Whiteboard:** line styles, opacity, a highlighter, nine colours plus a colour picker, and a text tool with α β θ π ° √ ² keys (Greek names like `alpha` are typeset). The first colour is now the theme's ink, because the old fixed near-black pen was invisible on the dark theme (saved layouts are migrated).
- **Derivations** the model writes appear line by line.
- The element schema is one described object, so the Math tools went from about 14.7K back to about 6.9K tokens (draw_diagram itself is about 1.2K), and Math still fits a 16K context.
- Tests: `tests/unit/math-diagram.test.ts` (18) covers geometry, all four quadrants of the preset, normalisation, colour sanitising, the whiteboard and exports. A session test covers `draw_diagram` + `addSteps` through the agent loop, and the Math e2e test now checks the step player, a dashed line and the text tool.
...
The Math rule carries over: **the model says what to write and which question it belongs to; Cellar works out where.** A local model cannot be trusted with page coordinates any more than with arithmetic.

- **Books** (`src/main/study/`, migration 12): `books` (the copy lives in `~/.cellar/books/<id>.pdf`, deduplicated by SHA-256), `book_pages` (each page's text as positioned lines), `book_fts` (BM25) and `book_vectors` (page embeddings when an embedding model is set, filled in the background). Importing reads every page once with `unpdf` (text runs grouped into lines that keep two columns apart, the outline resolved to page numbers, page sizes, the title from the metadata): about 2 ms a page for a text-only 300-page book. Password-protected and non-PDF files are refused with a sentence that says so.
- **Chats about a book**: conversations of kind `study` with `task.study = { bookId, mode, context }`. A book has any number of chats and they all share its notes; the reader opens the latest one and the title menu starts a new one or goes back to an earlier one.
- **What the model reads** (`context.ts`): a picker next to the composer — this page, from the chapter start to here (the outline gives the start), pages you type (`12-15, 20`), or the whole book — with a token estimate. Pages go in as text with everything written on them placed under the line it sits next to ("the user wrote "Newton""), plus which numbered questions on the page the user has answered and which are still open. A range too long for the budget keeps the page on screen and its neighbours and says which pages were left out; "whole book" on a real textbook becomes a search for the question (BM25 fused with embeddings). Selected text rides along as "this".
...
Tests: 412 unit tests (2 new files) and 22 Playwright tests (1 new). `tests/unit/study.test.ts` covers page lists and scopes, line grouping, finding questions (numbers, Turkish, paraphrases), every placement path, notes and marks, annotation checking and the page text the model reads. `tests/unit/study-session.test.ts` imports a real PDF drawn with pdf-lib and runs a scripted tutor through the agent loop in both modes (tools offered, marks after the user's answer, the refusal to write in Tutor mode, the blank found exactly), save conflicts, exporting and removing a book, plus a Segoe UI blank measured to within 0.1 pt. The Playwright test imports a worksheet, types an answer on the page, has a scripted tutor highlight, mark and leave a note, follows a page link, fills a blank in Solve mode, draws and undoes a pen stroke, sends two pages as context and exports.

Real software (throwaway profile, Ollama qwen3.5:9b on the RTX 5060, `scripts/study-smoke.mjs`): a generated Turkish science chapter with the student's answers already on page 3 (one right, two wrong, one left empty).
- **"Cevaplarımı kontrol eder misin?"** — three `mark_answer` calls straight away (✓ for 1, ✗ for 2 and 4 with "150÷3=50, 45 yanlış"), a short summary, and question 3 left for the student. 36 s.
- **Whole book: "Fotosentez nerede gerçekleşir…"** — answered from page 1 with a clickable (p. 1), and turned the viewer to that page itself (`go_to_page`). 18 s.
- **Solve: "3. sorudaki boşluğu doldur."** — "oksijen" written exactly on the blank. 22 s.
- **Export**: the copy with the notes, rendered with pdfium, shows the typed answers, the ✓/✗ marks and comments, and the filled blank, with Turkish letters intact.
- The first runs are why several rules exist: the model asked "should I mark them?" instead of marking (the prompt now says checking *is* marking), gave away question 3's answer (each page now states which questions are answered), marked its own answer (refused, with a message that ends the loop), and wrote "doğru ✓" as a comment next to its own ✓ (symbols and verdict-only comments are dropped). Answers first landed on the word before the blank — see the Segoe UI note below.

### Known gaps and follow-ups from M9
- **Form fields** in a PDF (AcroForm) are shown but cannot be filled in and saved; type on the page instead.
- **Blanks drawn as lines** (a vector rule rather than underscores or dots) are not detected as blanks; the answer goes under the question instead of on the line.
- **Scanned books** have no text to search or place against: the tutor reads them through page pictures (vision models only), and notes go in the margin. OCR on import would fix both.
- **Handwriting** is read only as a picture by a vision model; answer checking is reliable for typed answers.
- **Highlights the model makes** use estimated character widths inside a line (Arial metrics), so their ends can be half a character off in narrow or wide fonts; blanks are exact.
- **EPUB and other formats** are not books yet — only PDF.

### Technical notes learned in M9
- **pdf.js cannot fetch its own data files from `file://`.** Character maps, standard fonts and the JPEG 2000/JBIG2 decoders are loaded by URL, which fails for an app served from a file. pdf.js 6 takes a `BinaryDataFactory` (`fetch({ kind, filename })` → bytes) with `useWorkerFetch: false`, so the viewer asks the main process over IPC (`study:pdfAsset`), which reads them from `node_modules/pdfjs-dist`. That is why pdfjs-dist is a runtime dependency; `electron-builder.yml` drops its builds, types and legacy code, leaving about 4 MB.
- **`pdf_viewer.mjs` reads the library from `globalThis.pdfjsLib`.** Set it before importing the viewer module, and load the worker with Vite's `?worker` like Monaco's.
- **pdf.js clears unknown children of a page's div whenever it re-renders it** (`PDFPageView.reset`), so the annotation layer is re-attached on every `pagerendered` event and rendered through a React portal into that host.
