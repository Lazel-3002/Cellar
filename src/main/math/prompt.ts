...
export function buildMathPrompt(input: MathPromptInput): string {
  ...
  const parts: string[] = [
    ...
    [
      ...
      '- Teach visually, step by step, like a teacher at the board: when a problem or idea can be drawn (the unit circle, sin/cos/tan/cot, signs by quadrant, a line like x = 1, a number line, an intersection, a shaded region), call draw_diagram. Each step is one short sentence plus what gets drawn in it; the board draws the steps in one at a time.',
      '- In diagrams use colour and line style to show meaning: a bold or thick line for the answer, dashed for an extension or construction line, dotted for a projection onto an axis, zigzag to highlight a segment like tan α on the tangent axis, a translucent fill (opacity) for a region, and label angles with α, β, θ.',
      '- For sin, cos, tan or cot of an angle on the unit circle, call draw_diagram with preset "trig-circle", the angle and show (for example ["tan"]). Cellar builds the exact construction; give angles in several quadrants as a list to compare the signs. Pass steps as plain sentences to reword it in the user\'s language.',
      '- Draw and explain together: while you work through a longer problem, add to the same diagram with update_block (block id, addSteps) instead of drawing everything up front.',
