...
class ChatOrchestrator {
  ...
  getConversation(id: string): ConversationWithMessages {
    ...
    const messages = store.listMessages(id).map((m) => {
      ...
      return live ? { ...m, content: live.state.content, reasoning: live.state.reasoning, status: 'streaming' as const } : m;
    });
  ...
  async send(input: SendMessageInput): Promise<SendMessageResult> {
    ...
    const assistant = this.newAssistant(conversation.id, user.id, entry);
    ...
    store.updateConversation(conversation.id, {
      ...
      ...(conversation.title ? {} : { title: fallbackTitle(content || user.attachments[0]?.name || 'New chat') }),
    });
  ...
  async regenerate(conversationId: string, assistantMessageId: string, model?: ModelRef, thinking?: ThinkingLevel): Promise<SendMessageResult> {
    ...
    const assistant = this.newAssistant(conversationId, previous.parentId, entry);
    store.insertMessage(assistant);
    ...
    void this.generate(store, conversationId, assistant, entry, level, false);
  ...
  async edit(conversationId: string, userMessageId: string, content: string, model: ModelRef, thinking: ThinkingLevel): Promise<SendMessageResult> {
    ...
    store.insertMessage(user);
    store.indexMessage(user);
    const assistant = this.newAssistant(conversationId, user.id, entry);
    store.insertMessage(assistant);
    store.updateConversation(conversationId, { currentLeafId: assistant.id, model, settings: { ...conversation.settings, thinking } });
    this.notify(conversationId);
    void this.generate(store, conversationId, assistant, entry, thinking, false);
  ...
  private newAssistant(conversationId: string, parentId: string, entry: ModelEntry): Message {
    return {
      id: newId(),
      conversationId,
      parentId,
